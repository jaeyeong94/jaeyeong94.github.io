  ...
  experience: {
    ...
    items: {
      masblue: {
        ...
        bullets: [
          '米国保険会社のバックオフィス業務をLLMエージェントに分解しオーケストレーションするバックエンドを設計。各ステップにドメインのガードレール、Human-in-the-Loopチェックポイント、監査ログを組み込み、自動化と規制対応を同時に満たす(指標:パイロット処理時間ダッシュボード)。',
      ...
      onto: {
        ...
        bullets: [
          'CCTV(RTSP) → オンサイトGPU AI推論 → Cloudflare R2アップロードのエンドツーエンドパイプラインを設計。ネットワーク断絶時は最大24時間のローカルバッファと再アップロードでデータ欠落0%を目標(指標:Cloudflare Analytics、7日平均)。',
          'Presigned URL + コールバックベースのアップロードAPI設計により、カメラN台追加時のバックエンドデプロイを0件に。ノードはシークレット認証のみで横スケール(指標:内部デプロイログ)。',
        ],
      },
      rootstone: {
        company: 'Rootstone',
        bullets: [
          'B2Bトレーディング成果レポートシステムを設計・運用。元帳分離 + 日次/月次スナップショットバッチ + チェックサム検証のPDFパイプラインで正確性と監査性を確保(指標:バッチ成功率 [CloudWatch])。',
          'Electron + Node.js + PostgreSQL + Redis + WebSocketベースで暗号資産アービトラージ / マーケットメイキングエンジンを構築。受信/戦略/発注のイベントループを分離し、戦略実行p95レイテンシをベースライン比で短縮(指標:内部レイテンシログ)。',
        ],
      },
      dio: {
        company: 'DIO フリーランスプラットフォーム',
        bullets: [
          '(Woo-Yeon-Hee) 位置ベースのマッチングアプリのマッチング・チェックアウト・自動精算ロジックをステートマシン + イベントソーシングでモデリングし、8週で本番投入。主要フローの欠陥0件でローンチ(指標:Sentry)。',
          '(Linker) Toss PGとカカオAlimTalkをOutboxパターンで結合し決済–通知の整合性を確保。リトライ + DLQフローで決済失敗率を約1%に安定化(指標:PG Webhookログ、30日平均)。',
          '(Momentir) LLMベースの顧客ノート要約 + スケジュール自動抽出パイプラインを設計。構造化結果をエージェント承認UIで提供しCRM漏れを最小化、パイロット12名で日次運用工数が約30%削減(手法:ユーザーインタビュー)。',
          '(Hidden Gem) PostGIS + Redisベースの位置・カテゴリフィード・バックエンドを設計。インデックス・キャッシュ戦略でメインフィードp95応答時間を目標内に維持(指標:APM p95、7日平均)。',
        ],
      },
      dssolve: {
        company: 'DS Solve',
      ...
      aligo: {
        company: 'ALIGO',
        bullets: [
          'Vanilla PHP 7ベースの業務メッセージ送信API・サーバーを開発・運用。顧客別キュー分割と加重ラウンドロビンでピーク時間の公平性を改善、リトライは指数バックオフで分離キュー処理(指標:社内送信キューダッシュボード)。',
      ...
      'studio-bloom': {
        ...
        bullets: [
          'WordpressプラグインベースのP2P貸付ソリューションを開発、Paygate Seyfert Fintech API(仮想口座・投資・返済・ステータス)と連携。トランザクション分離 + 冪等レスポンス保存で運用期間中の金融整合性インシデント0件(指標:日次整合性チェックバッチ)。',
        ],
      },
      ninefive: {
        company: 'Ninefive',
      ...
      '4depth': {
        company: '4depth',
        bullets: [
          '官公庁 · 自治体向けサイトのパブリッシング、WCAGアクセシビリティ対応、レスポンシブWeb実装。セマンティックHTMLとARIA設計、デザイン段階からのアクセシビリティチェックリストで、複数のプロジェクトでアクセシビリティ認証を通過。',
        ],
      },
    },
  },
